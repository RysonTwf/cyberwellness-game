...
Fable Falls, now that it's enabled — stays inline SVG in
...
for what's genuinely new and file-based:
...
- Fable Falls' Detective/Compare clue assets (`fablefalls-detective/`) and
  ...
  `src/data/realms.js` for what's still a placeholder within that (the
...
- A designer starting the P4–P6 platformer/stepping-stone art now (Phase 2
  can begin before that art lands, per Milestones — "don't let any
