...
## Changelog
- **26 Aug 2026: touch path for walking restored — the Phase 4 blocker.** `world/useWalker.js` gained `press(code)`/`release(code)`, sharing the same `KEY_DIRS` codes and `keys` set the keyboard listener already used — so the tick loop, the input-lock check, and the blur/visibilitychange stuck-key safety net all cover touch presses for free. `world/World.jsx` renders a `.dpad`: four ≥48px buttons (up/left/right/down) fixed in the scene's bottom-left corner, held via mouse *and* touch handlers (same `onMouseDown/Up/Leave` + `onTouchStart/End/Cancel` pattern as `minigames/MiniGamePlatformer.jsx`'s left/jump/right controls), each calling `press`/`release` on pointer down/up. Deliberately a dedicated corner control, not tap-anywhere movement — that's what competed with hotspot/UI clicks and got click-to-move pulled in `ef16cce` in the first place. Covers all three walking layers (Traveler's Room, Atlas hub, every realm) since they all route through `World.jsx`. Verified with `npm run build`.
