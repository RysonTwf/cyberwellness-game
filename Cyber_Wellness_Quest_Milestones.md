...
- [ ] Stepping-stone/fog assets for Privacy Peaks P4–P6 — same sprite sheet format. **The manifest pass is done (26 Aug 2026)** — `phaser-scenes/steppingStonesArt.js` now declares `ss-stone` (50×30), `ss-token` (20×26), and `ss-fog` (560×220), same shape as `passworldArt.js`. Final art itself is still not started — no skin has been added to `SKINS` yet (unlike Passworld, nothing's been trialed here), so this is a designer task now unblocked rather than a finished one.
...
**Design rule added 19 Aug 2026 — a mini-game must not be passable without judgement.** An audit of all five found that four could be finished without deciding anything, which quietly made them typing exercises rather than cyber-wellness ones. Sort filed each card into its *correct* bin whichever one you picked, so hitting any bin every time still reached "Done". Spot revealed whether a message was a real problem the instant you tapped it, so tapping all of them scored full marks. Stepping Stones advanced regardless of the call, so "step" on everything crossed the ravine. Passworld's platformer won outright the moment the last tile was touched. All four now require the judgement to be right before they'll complete, with free retries and no fail state either way (design.md §8) — the fix is never a punishment, it's removing the path that skipped the thinking. **Balance Bay is the deliberate exception:** it has no correct answer by design (a values exercise, not a right/wrong one), so gating it would misrepresent what it teaches. Any new mechanic should be checked against this rule before it ships.
...
4. **Stepping-stone/fog art** for Privacy Peaks — **unblocked 26 Aug 2026:** `steppingStonesArt.js` now gives you a measured list (`ss-stone` 50×30, `ss-token` 20×26, `ss-fog` 560×220), same pattern as Passworld's manifest, just with no CC0 pack trialed in yet — you're skinning against the procedural stand-ins directly.
...
4. ~~Restore a touch path for walking~~ — **done 26 Aug 2026.** Movement was keyboard-only as of `ef16cce`, which made the overworld unplayable on a tablet. See Phase 4.
5. ~~Give the stepping-stone scene the same `ART_MANIFEST`/skin treatment as Passworld~~ — **done 26 Aug 2026.** See changelog.
...
## Changelog
- **26 Aug 2026: stepping-stone scene given the same `ART_MANIFEST`/skin treatment as Passworld.** New `phaser-scenes/steppingStonesArt.js`, same shape as `passworldArt.js`: `ART_MANIFEST` declares `ss-stone` (50×30), `ss-token` (20×26, the hop marker), and `ss-fog` (560×220) — each with a procedural `draw()` stand-in — plus `SKINS`/`ACTIVE_SKIN` ready for a real pack the same way Passworld's `brackeys` skin works, currently just `{ builtin: null }` since nothing's been trialed here yet. `steppingStonesScene.js` was rewired to load through it (`preload()`/`create()` calling `preloadSteppingStonesArt`/`buildSteppingStonesArt`, same call shape as the platformer scene) instead of building its three textures inline with raw Phaser `Graphics.generateTexture` calls. The fog also changed *how* it's drawn in the process — several soft radial-gradient blobs instead of a single flat 40%-opacity rectangle — so there's an actual texture shape for real fog art to replace rather than a plain tint; visual behavior otherwise unchanged. Verified with `npm run build`; a manual look in the running realm (Privacy Peaks P4–P6, "Clear the Fog: Level Up") is still open to confirm the fog re-render looks right.
- **26 Aug 2026: touch path for walking restored — the Phase 4 blocker.** First pass added an on-screen d-pad; revised same-day to **tap/click-to-move** instead, on request — the player taps where they want to go, and the existing interact button still pops up once they're close enough to a hotspot, exactly as it already did regardless of how the Traveler got there. `world/useWalker.js` gained `goTo(x, y)`: a one-shot destination the tick loop steers toward each frame (same axis-separated step/clamp/obstacle logic keyboard movement already used, just aimed at a point instead of a held direction), snapping to arrival within `ARRIVE_EPS` and giving up if blocked rather than sitting stuck forever. A real key press cancels a pending tap-target — keyboard always wins if both are in play. `world/World.jsx` reads the tap position off `.world`'s own bounding rect and calls `goTo`. This is the second attempt at click-to-move (the first, pre-`ef16cce`, got pulled for competing with clicks on hotspots/UI — tapping the interact button also moved the Traveler out from under it): this time the handler sits on `.world` itself, and everything clickable layered over the scene (currently just the interact button) already stops the event at `onPointerDown` before it reaches that background — the same guard the button already had. Verified with `npm run build`, and manually in Chrome mobile emulation (the automation tool couldn't reach the browser in this environment, so this was a manual check) — confirmed working.
